...
from .const import (
    ...
    NUM_ROWS)
from .utils import (
    assert_model, create_run_info, create_run_variables, create_schema,
    serialize_data)
...
async def mocked_latest_rows(mocker, current_timestamp):
    def mocked_returns(*args, table, **kwargs):
        if table == 'run_variables':
            return (create_run_variables(NEW_VALUES,
...
@pytest.mark.asyncio
async def test_latest_data(graphql_schema,
                           current_timestamp,
                           mocked_latest_rows,
                           mocked_new_count):
    ...
    async for result in subscription:
        ...
        assert metadata['schema'] == create_schema(dtypes)
